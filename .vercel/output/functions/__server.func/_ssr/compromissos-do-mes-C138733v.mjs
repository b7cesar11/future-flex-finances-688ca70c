...
import { A as Clock, E as Landmark, M as ChevronRight, N as ChevronLeft, O as CreditCard, Y as CircleCheck, c as TrendingDown, i as Users, s as TrendingUp } from "../_libs/lucide-react.mjs";
import { t as AppShell } from "./AppShell-C8xxjcGv.mjs";
import { t as PayCheckbox } from "./PayCheckbox-BiXeFJkl.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/compromissos-do-mes-C138733v.js
...
function originIcon(tipo) {
	switch (tipo) {
		...
		case "receita": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TrendingUp, { className: "h-4 w-4" });
	}
}
...
function CompromissosDoMes() {
	...
	const frozenGroupIds = (0, import_react.useMemo)(() => {
		const s = /* @__PURE__ */ new Set();
		for (const d of dividas) if (d.frozenAt && d.commitmentGroupId) s.add(d.commitmentGroupId);
		return s;
	}, [dividas]);
	...
	const gruposParcelados = (0, import_react.useMemo)(() => {
		...
		for (const t of transacoes) {
			if (!(t.purchaseGroupId ?? t.commitmentGroupId)) continue;
			if (t.invoiceId) continue;
			if (t.kind === "despesa") {} else if (t.kind === "receita") {} else continue;
			...
			if (t.kind === "receita") {
				if (!byGroup.has("__receitas__")) byGroup.set("__receitas__", {
					tipo: "receita",
					titulo: "Receitas",
					itens: [],
					total: 0,
