...
//#region node_modules/.nitro/vite/services/ssr/assets/finance-store-C9XWVt39.js
...
function FinanceProvider({ children }) {
	...
	const payDebtM = useMutation({
		mutationFn: async (id) => {
			const { error } = await supabase.rpc("pay_debt_installment", { _debt_id: id });
			if (error) throw error;
		},
		onSuccess: invalidateAll
	});
	const revertDebtM = useMutation({
		mutationFn: async (id) => {
			const { error } = await supabase.rpc("revert_debt_payment", { _debt_id: id });
			if (error) throw error;
		},
		onSuccess: invalidateAll
	});
	const payDebtWithAmountM = useMutation({
		mutationFn: async ({ id, amount, accountId }) => {
			...
			const { data: debtRow, error: dErr } = await supabase.from("debts").select("name").eq("id", id).maybeSingle();
			if (dErr) throw dErr;
			const { error: rpcErr } = await supabase.rpc("pay_debt_with_amount", {
				_debt_id: id,
				_amount: amount
			});
	...
	const value = (0, import_react.useMemo)(() => {
		...
		const dividas = (debtsQ.data ?? []).map((r) => {
			...
			return {
				...
				parcelasTotais,
				tipo: r.type,
				category: r.category ?? (r.is_variable ? "variavel" : "parcelada"),
		...
		return {
			...
			payDebtWithAmount: async (id, amount, accountId) => {
				await payDebtWithAmountM.mutateAsync({
					id,
					amount,
					accountId
				});
