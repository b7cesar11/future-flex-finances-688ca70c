...
function Receitas() {
  const { fontesRenda, contas, addIncomeSource, setIncomeStatus, deleteIncomeSource } =
    useFinance();
    ...
    <AppShell title="Fontes de renda" subtitle="Salários, VA e outros recebimentos">
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          <button aria-label="Fechar" className="absolute inset-0 bg-black/60" onClick={() => setOpen(false)} />
          <form
            onSubmit={submit}
            className="relative mx-auto w-full max-w-md space-y-3 rounded-t-3xl bg-card p-5 pb-8"
          >
            <h3 className="text-base font-semibold">Nova fonte de renda</h3>
            <input
              value={name}
